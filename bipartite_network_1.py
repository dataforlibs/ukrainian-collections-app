...
def read_data(filepath):
    """Read keywords and dates, build bipartite edges."""
    edges = defaultdict(int)  # (keyword, date) -> count
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            coll = (row.get('Ключевые слова') or '').strip()
    ...
    print(f"Keywords: {len(coll_totals)}")
...
def build_transposed(edges, coll_totals):
    ...
    print(f"Transposed edges (keyword pairs): {len(coll_edges)}")
    return coll_edges
...
<header>
  <div class="title-block">
    ...
    <h2 id="subtitle">Materials/Keywords ↔ Time Periods (bipartite)</h2>
  </div>
  <div class="mode-buttons">
    <button class="mode-btn active" id="btnBipartite">Materials ↔ Periods</button>
    <button class="mode-btn" id="btnTransposed">Material ↔ Material</button>
  </div>
</header>
...
<div id="info-bar">
  ...
  <span>Materials: <span class="info-val">{len(sorted_colls)}</span></span>
...
function drawBipartite() {{
  g.selectAll('*').remove();
  document.getElementById('subtitle').textContent = 'Materials/Keywords ↔ Time Periods (bipartite)';
...
function drawTransposed() {{
  g.selectAll('*').remove();
  document.getElementById('subtitle').textContent = 'Material ↔ Material (shared time periods)';
  ...
  nodeGroups.on('mouseover', (event, d) => {{
    ...
    showTooltip(event, `
      ...
      <div><span style="color:#4ade80">■</span> Hermitage: ${{d.hermitage.toLocaleString()}}</div>
      <div><span style="color:#60a5fa">■</span> SHM: ${{d.shm.toLocaleString()}}</div>
      <div class="tt-detail">Connected to ${{connCount}} other materials via shared time periods</div>
    `);
  }})
  .on('mouseout', () => {{ resetHighlight(); hideTooltip(); }});
}}
