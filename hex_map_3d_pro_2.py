...
function getHeight(total) {{
  ...
  return (Math.log(total + 1) / Math.log(MAX_TOTAL + 1)) * 8 * heightMultiplier;
}}

function buildPillars() {{
  // Remove old
  hexMeshes.forEach(m => {{
    if (m.hermMesh) scene.remove(m.hermMesh);
    if (m.shmMesh) scene.remove(m.shmMesh);
    if (m.emptyMesh) scene.remove(m.emptyMesh);
  }});
  hexMeshes = [];

  const gapVal = parseFloat(document.getElementById('gapSlider').value);
  const scaledShape = createHexShape(HEX_SIZE * SCALE * gapVal);
  ...
  hexData.forEach(d => {{
    ...
    if (d.total === 0) {{
      const geo = new THREE.ExtrudeGeometry(scaledShape, {{
      ...
      mesh.userData = d;
      scene.add(mesh);
      entry.emptyMesh = mesh;
    }} else {{
      const totalH = getHeight(d.total);
      const hermH = (d.hermitage / d.total) * totalH;
      const shmH = totalH - hermH;

      // Hermitage (bottom)
      if (hermH > 0.01) {{
        const geo = new THREE.ExtrudeGeometry(scaledShape, {{
          depth: hermH, bevelEnabled: true,
          bevelThickness: 0.02, bevelSize: 0.02, bevelSegments: 2
        }});
        const mesh = new THREE.Mesh(geo, hermMat.clone());
        mesh.rotation.x = -Math.PI / 2;
        mesh.position.set(x, 0, -z);
        ...
        // Glowing top cap for hermitage section
        const capGeo = new THREE.ExtrudeGeometry(scaledShape, {{
          depth: 0.03, bevelEnabled: false
        }});
        const capMesh = new THREE.Mesh(capGeo, topHermMat);
        capMesh.rotation.x = -Math.PI / 2;
        capMesh.position.set(x, hermH, -z);
        capMesh.userData = d;
        scene.add(capMesh);
        entry.hermCap = capMesh;
      }}

      // SHM (top)
      if (shmH > 0.01) {{
        const geo = new THREE.ExtrudeGeometry(scaledShape, {{
          depth: shmH, bevelEnabled: true,
          bevelThickness: 0.02, bevelSize: 0.02, bevelSegments: 2
        }});
        const mesh = new THREE.Mesh(geo, shmMat.clone());
        mesh.rotation.x = -Math.PI / 2;
        mesh.position.set(x, hermH, -z);
        mesh.castShadow = true;
        mesh.receiveShadow = true;
        mesh.userData = d;
        scene.add(mesh);
        ...
        const capMesh = new THREE.Mesh(capGeo, topShmMat);
        capMesh.rotation.x = -Math.PI / 2;
        capMesh.position.set(x, totalH, -z);
        capMesh.userData = d;
        scene.add(capMesh);
        entry.shmCap = capMesh;
      }}
    }}
...
function updateTooltip(e) {{
  ...
  const allMeshes = [];
  hexMeshes.forEach(h => {{
    if (h.hermMesh) allMeshes.push(h.hermMesh);
    if (h.shmMesh) allMeshes.push(h.shmMesh);
    if (h.hermCap) allMeshes.push(h.hermCap);
    if (h.shmCap) allMeshes.push(h.shmCap);
    if (h.emptyMesh) allMeshes.push(h.emptyMesh);
  }});
...
document.getElementById('heightSlider').addEventListener('input', function() {{
  ...
  hexMeshes.forEach(m => {{
    ...
    if (m.shmMesh) scene.remove(m.shmMesh);
    if (m.emptyMesh) scene.remove(m.emptyMesh);
    if (m.hermCap) scene.remove(m.hermCap);
    if (m.shmCap) scene.remove(m.shmCap);
  }});
  buildPillars();
}});
