...
def date_to_period(year):
    if year < -3000: return "Pre-3000 BCE"
    elif year < -1000: return f"{int(year // 1000 * 1000)} BCE"
    elif year < -500: return "1000–500 BCE"
    elif year < -300: return "500–300 BCE"
    elif year < -100: return "300–100 BCE"
    elif year < 0: return "100–1 BCE"
    elif year < 100: return "1–100 CE"
    elif year < 300: return "100–300 CE"
    elif year < 500: return "300–500 CE"
    elif year < 700: return "500–700 CE"
    elif year < 900: return "700–900 CE"
    elif year < 1100: return "900–1100 CE"
    elif year < 1300: return "1100–1300 CE"
    elif year < 1500: return "1300–1500 CE"
    elif year < 1700: return "1500–1700 CE"
    elif year < 1800: return "1700–1800 CE"
    elif year < 1900: return "1800–1900 CE"
    else: return "1900+ CE"


PERIOD_ORDER = [
    "Pre-3000 BCE",
    "-5000 BCE", "-4000 BCE", "-3000 BCE", "-2000 BCE", "-1000 BCE",
...
def read_data(filepath):
    # Per period: which keywords appear, and co-occurrence
    period_keywords = defaultdict(lambda: defaultdict(int))  # period -> keyword -> count
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            period_keywords[period][kw] += 1
            keyword_totals[kw] += 1
            if museum == 'shm':
                period_museum[period][kw]['shm'] += 1
            else:
                period_museum[period][kw]['hermitage'] += 1

    # Filter periods that exist in data, keep order
    active_periods = [p for p in PERIOD_ORDER if p in period_keywords]
...
def build_temporal_snapshots(period_keywords, period_museum, keyword_totals, active_periods):
    """Build cumulative network state at each time step."""
    snapshots = []
    cumulative_kw = defaultdict(int)  # keyword -> cumulative count
    cumulative_museum = defaultdict(lambda: {'hermitage': 0, 'shm': 0})
    first_seen = {}
    ...
    for pi, period in enumerate(active_periods):
        ...
        for kw, count in kws_this_period.items():
            ...
            cumulative_kw[kw] += count
            mus = period_museum[period][kw]
            cumulative_museum[kw]['hermitage'] += mus['hermitage']
            cumulative_museum[kw]['shm'] += mus['shm']

        # Build nodes: all keywords seen so far
        nodes = []
        for kw in cumulative_kw:
            nodes.append({
                'id': kw,
                'label': kw,
                'total': cumulative_kw[kw],
                'thisperiod': kws_this_period.get(kw, 0),
                'hermitage': cumulative_museum[kw]['hermitage'],
                'shm': cumulative_museum[kw]['shm'],
                'firstSeen': first_seen[kw],
                'isNew': kw in new_keywords,
            })

        # Build edges: co-occurrence in this period
        # Two keywords are linked if they both appear in this period
        kw_list = [kw for kw in kws_this_period]
        period_edges = []
        ...
        # Cumulative edges (all co-occurrences up to now)
        cum_cooccur = defaultdict(int)
        for pp in active_periods[:pi + 1]:
            kws_pp = list(period_keywords[pp].keys())
            for ii in range(len(kws_pp)):
                for jj in range(ii + 1, len(kws_pp)):
                    pair = tuple(sorted([kws_pp[ii], kws_pp[jj]]))
                    cum_cooccur[pair] += min(period_keywords[pp][kws_pp[ii]], period_keywords[pp][kws_pp[jj]])

        cum_edges = []
        for (k1, k2), w in cum_cooccur.items():
            cum_edges.append({'source': k1, 'target': k2, 'weight': w})

        artifact_count = sum(kws_this_period.values())
        cum_count = sum(cumulative_kw.values())
        ...
        snapshots.append({
            ...
            'nodes': nodes,
            'edges': cum_edges,
            'periodEdges': period_edges,
            'newKeywords': new_keywords,
            'artifactsThisPeriod': artifact_count,
            'cumulativeArtifacts': cum_count,
            'nodeCount': len(nodes),
            'edgeCount': len(cum_edges),
        })
...
<header>
  <div class="title-block">
    ...
    <h2>How artifact materials connect and evolve across historical periods</h2>
  </div>
</header>
...
<div id="timeline">
  <div id="timeline-header">
    ...
    <div id="timeline-stats">
      ...
      <span>This period: <span class="ts-val ts-new" id="sThis">0</span></span>
      <span>Cumulative: <span class="ts-val ts-gold" id="sCum">0</span></span>
      <span>New: <span class="ts-val ts-new" id="sNew">0</span></span>
    </div>
  </div>
  <div id="controls-row">
    <button class="play-btn" id="btnPlay">▶</button>
    <input type="range" id="scrubber" min="0" max="0" value="0" step="1">
    <button class="speed-btn active" data-speed="2000">1×</button>
    <button class="speed-btn" data-speed="1000">2×</button>
    <button class="speed-btn" data-speed="500">4×</button>
  </div>
...
snapshots.forEach((s, i) => {{
  ...
  mark.textContent = s.period.replace(' CE','').replace(' BCE','');
...
function updateVisualization(step) {{
  ...
  document.getElementById('sCum').textContent = snap.cumulativeArtifacts.toLocaleString();
  ...
  nodeElements.on('mouseover', (event, d) => {{
    ...
    tooltip.html(`
      ...
      <div style="color:#8b949e;margin-top:4px">This period: ${{d.thisperiod.toLocaleString()}}</div>
      <div style="color:#8b949e">First appeared: ${{periods[d.firstSeen]}}</div>
    `)
    .style('display', 'block')
